...
class SalesforceProcessor:
    def __init__(self, report_name, bucket_name):
        ...
        self.sf = Salesforce(instance='platform-platform-801-dev-ed.scratch.my.salesforce.com', session_id=self.access_token)
    ...
    def process_organizations(self):
        ...
        if self.account_list:
            print(self.sf.bulk.Account.upsert(self.account_list, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True))  
            
        if self.phone_list:
            print(self.sf.bulk.vnfp__Legacy_Data__c.upsert(self.phone_list, 'vnfp__Implementation_External_ID__c', batch_size='auto',use_serial=True))
            
        if self.address_list:
            print(self.sf.bulk.npsp__Address__c.upsert(self.address_list, 'vnfp__Implementation_External_ID__c', batch_size='auto',use_serial=True))

        if self.phone_act_list:
            print(self.sf.bulk.Account.upsert(self.phone_act_list, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True)) 
            
        if self.address_act_list:
            print(self.sf.bulk.Account.upsert(self.address_act_list, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True)) 
    ...
    def process_households(self):
        ...
        if self.houseHolds_list:
            print(self.sf.bulk.Account.upsert(self.houseHolds_list, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True))
    ...
    def process_contacts(self):
        ...
        if self.contacts_phones_list:
            print(self.sf.bulk.vnfp__Legacy_Data__c.upsert(self.contacts_phones_list, 'vnfp__Implementation_External_ID__c', batch_size='auto',use_serial=True)) 

        if self.contacts_emails_list:
            print(self.sf.bulk.vnfp__Legacy_Data__c.upsert(self.contacts_emails_list, 'vnfp__Implementation_External_ID__c', batch_size='auto',use_serial=True)) 
            
        if self.contacts_act_phone:
            print(self.sf.bulk.Contact.upsert(self.contacts_act_phone, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True))
            
        if self.contacts_act_email:
            print(self.sf.bulk.Contact.upsert(self.contacts_act_email, 'Auctifera__Implementation_External_ID__c', batch_size='auto',use_serial=True))
    ...
    def process_contact_address(self):
        ...
        if self.contacts_address_list:
            print(self.sf.bulk.npsp__Address__c.upsert(self.contacts_address_list, 'vnfp__Implementation_External_ID__c', batch_size='auto',use_serial=True))  
